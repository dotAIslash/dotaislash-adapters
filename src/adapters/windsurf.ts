 ...
 */
export class WindsurfAdapter extends BaseAdapter<string> {
  ...
  transform(context: Context, options?: AdapterOptions): string {
    ...
    if (context.settings) {
      const features: WindsurfConfig['features'] = {};
      
      // Check for Windsurf-specific settings
      if ('streaming' in context.settings) {
        features.streaming = context.settings.streaming as boolean;
      }
      if ('autocomplete' in context.settings) {
        features.autocomplete = context.settings.autocomplete as boolean;
      }
      if ('inlineEdit' in context.settings) {
        features.inlineEdit = context.settings.inlineEdit as boolean;
      }
